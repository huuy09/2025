...
# Hyper Parameters:
cdef double GAMMA = 1.0  # decay rate of past observations
cdef int UPDATE_TIME = 1000
cdef int EMBEDDING_SIZE = 64
cdef int MAX_ITERATION = 100000
cdef double LEARNING_RATE = 0.00005   #dai
...
cdef int N_STEP_REPLAY = 1
cdef int N_STEP_ACTION = 3
cdef int NUM_MIN = 20
cdef int NUM_MAX = 40
...
cdef int aggregatorID = 2 #0:sum; 1:mean; 2:GCN
...
class FINDER_BO:
    ...
    def __init__(self):
        ...
        for i in range(num_env):
            ...
            self.g_dual_list.append(graph.py_Graph())
            self.adj_list.append(np.zeros((1, 1)))
        ...
        # [batch_size, edge_cnt]
        self.action_select_dual = tf.sparse_placeholder(tf.float32, name="action_select_dual")
        # [edge_cnt, batch_size]
        self.rep_global_dual = tf.sparse_placeholder(tf.float32, name="rep_global_dual")
        # [edge_cnt, batch_size]
        self.n2nsum_param_dual = tf.sparse_placeholder(tf.float32, name="n2nsum_param")
        # [edge_cnt, edge_cnt]
        self.laplacian_param_dual = tf.sparse_placeholder(tf.float32, name="laplacian_param_dual")
        # [batch_size, edge_cnt]
        self.subgsum_param_dual = tf.sparse_placeholder(tf.float32, name="subgsum_param")
        # [batch_size,1]
        # self.target_edge = tf.placeholder(tf.float32, [BATCH_SIZE,1], name="target_edge")
        # [batch_size, aux_dim]
        self.aux_input_dual = tf.placeholder(tf.float32, name="aux_input_dual")
        # [edge_cnt, 2]
        self.node_input_dual = tf.placeholder(tf.float32, name="node_input_dual")

        # [batch_size, 1]
        # self.node_loss = tf.placeholder(tf.float32, name="node_loss")
        # self.edge_loss = tf.placeholder(tf.float32, name="edge_loss")
        self.node_actions = tf.placeholder(tf.float32, name="node_actions")
        self.edge_actions = tf.placeholder(tf.float32, name="edge_actions")
        ...
        # init Q network
        self.loss,self.trainStep,self.q_pred, self.q_on_all, self.q_on_all_edge,self.Q_param_list = self.BuildNet() #[loss,trainStep,q_pred, q_on_all, ...]
        #init Target Q Network
        self.lossT,self.trainStepT,self.q_predT, self.q_on_allT, self.q_on_allT_edge,self.Q_param_listT = self.BuildNet()
        #takesnapsnot
...
#################################################New code for FINDER#####################################
    def BuildNet(self):
        '''weight initialize'''
        # [2, embed_dim]
        w_n2l_node = tf.Variable(tf.truncated_normal([2, self.embedding_size], stddev=initialization_stddev), tf.float32)
        # [embed_dim, embed_dim]
        p_node_conv_node = tf.Variable(tf.truncated_normal([self.embedding_size, self.embedding_size], stddev=initialization_stddev), tf.float32)
        if embeddingMethod == 1:    #'graphsage'
            # [embed_dim, embed_dim]
            p_node_conv2_node = tf.Variable(tf.truncated_normal([self.embedding_size, self.embedding_size], stddev=initialization_stddev), tf.float32)
            # [2*embed_dim, embed_dim]
            p_node_conv3_node = tf.Variable(tf.truncated_normal([2*self.embedding_size, self.embedding_size], stddev=initialization_stddev), tf.float32)
        ...
        # no sparse
        input_message = tf.matmul(tf.cast(self.node_input,tf.float32), w_n2l_node)
        #[node_cnt, embed_dim]  # no sparse
        input_potential_layer = tf.nn.relu(input_message)

        # # no sparse
        # [batch_size, embed_dim]
        y_input_message = tf.matmul(tf.cast(y_node_input,tf.float32), w_n2l_node)
        ...
        while lv < max_bp_iter:
            ...
            node_linear = tf.matmul(n2npool, p_node_conv_node)
            ...
            y_node_linear = tf.matmul(y_n2npool, p_node_conv_node)
            ...
            if embeddingMethod == 0: # 'structure2vec'
            ...
            elif embeddingMethod == 1:   # 'graphsage'
                #[node_cnt, embed_dim] * [embed_dim, embed_dim] = [node_cnt, embed_dim], dense
                cur_message_layer_linear = tf.matmul(tf.cast(cur_message_layer, tf.float32), p_node_conv2_node)
                ...
                cur_message_layer = tf.nn.relu(tf.matmul(merged_linear, p_node_conv3_node))

                #[batch_size, embed_dim] * [embed_dim, embed_dim] = [batch_size, embed_dim], dense
                y_cur_message_layer_linear = tf.matmul(tf.cast(y_cur_message_layer, tf.float32), p_node_conv2_node)
                ...
                y_cur_message_layer = tf.nn.relu(tf.matmul(y_merged_linear, p_node_conv3_node))
        ...
        '''edge embedding'''
        # [2, embed_dim]
        w_n2l_edge = tf.Variable(tf.truncated_normal([2, self.embedding_size], stddev=initialization_stddev), tf.float32)
        # [embed_dim, embed_dim]
        p_node_conv_edge = tf.Variable(tf.truncated_normal([self.embedding_size, self.embedding_size], stddev=initialization_stddev), tf.float32)
        if embeddingMethod == 1:    #'graphsage'
            # [embed_dim, embed_dim]
            p_node_conv2_edge = tf.Variable(tf.truncated_normal([self.embedding_size, self.embedding_size], stddev=initialization_stddev), tf.float32)
            # [2*embed_dim, embed_dim]
            p_node_conv3_edge = tf.Variable(tf.truncated_normal([2*self.embedding_size, self.embedding_size], stddev=initialization_stddev), tf.float32)

        # #[node_cnt, 2]
        # nodes_size = tf.shape(self.n2nsum_param)[0]
        # node_input = tf.ones((nodes_size,2))
        y_edges_size = tf.shape(self.subgsum_param_dual)[0]
        # [batch_size, 2]
        y_edge_input = tf.ones((y_edges_size,2))

        #[node_cnt, 2] * [2, embed_dim] = [node_cnt, embed_dim]
        # no sparse
        input_message_edge = tf.matmul(tf.cast(self.node_input_dual, tf.float32), w_n2l_edge)
        #[node_cnt, embed_dim]  # no sparse
        input_potential_layer_edge = tf.nn.relu(input_message_edge)

        # # no sparse
        # [batch_size, embed_dim]
        y_input_message_edge = tf.matmul(tf.cast(y_edge_input,tf.float32), w_n2l_edge)
        #[batch_size, embed_dim]  # no sparse
        y_input_potential_layer_edge = tf.nn.relu(y_input_message_edge)

        #input_potential_layer = input_message
        lv = 0
        #[node_cnt, embed_dim], no sparse
        cur_message_layer_edge = input_potential_layer_edge
        cur_message_layer_edge = tf.nn.l2_normalize(cur_message_layer_edge, axis=1)

        #[batch_size, embed_dim], no sparse
        y_cur_message_layer_edge = y_input_potential_layer_edge
        # [batch_size, embed_dim]
        y_cur_message_layer_edge = tf.nn.l2_normalize(y_cur_message_layer_edge, axis=1)


        while lv < max_bp_iter:
            lv = lv + 1
            #[node_cnt, node_cnt] * [node_cnt, embed_dim] = [node_cnt, embed_dim], dense
            n2npool_edge = tf.sparse_tensor_dense_matmul(tf.cast(self.n2nsum_param_dual,tf.float32), cur_message_layer_edge)
            #[node_cnt, embed_dim] * [embed_dim, embed_dim] = [node_cnt, embed_dim], dense
            edge_linear = tf.matmul(n2npool_edge, p_node_conv_edge)

            # [batch_size, node_cnt] * [node_cnt, embed_dim] = [batch_size, embed_dim]
            y_n2npool_edge = tf.sparse_tensor_dense_matmul(tf.cast(self.subgsum_param_dual,tf.float32), cur_message_layer_edge)
            #[batch_size, embed_dim] * [embed_dim, embed_dim] = [batch_size, embed_dim], dense
            y_edge_linear = tf.matmul(y_n2npool_edge, p_node_conv_edge)

            if embeddingMethod == 0: # 'structure2vec'
                #[node_cnt, embed_dim] + [node_cnt, embed_dim] = [node_cnt, embed_dim], return tensed matrix
                merged_linear = tf.add(edge_linear,input_message_edge)
                #[node_cnt, embed_dim]
                cur_message_layer_edge = tf.nn.relu(merged_linear)

                #[batch_size, embed_dim] + [batch_size, embed_dim] = [batch_size, embed_dim], return tensed matrix
                y_merged_linear = tf.add(y_edge_linear, y_input_message_edge)
                #[batch_size, embed_dim]
                y_cur_message_layer_edge = tf.nn.relu(y_merged_linear)
            elif embeddingMethod == 1:   # 'graphsage'
                #[node_cnt, embed_dim] * [embed_dim, embed_dim] = [node_cnt, embed_dim], dense
                cur_message_layer_linear_edge = tf.matmul(tf.cast(cur_message_layer_edge, tf.float32), p_node_conv2_edge)
                #[[node_cnt, embed_dim] [node_cnt, embed_dim]] = [node_cnt, 2*embed_dim], return tensed matrix
                merged_linear = tf.concat([edge_linear, cur_message_layer_linear_edge], 1)
                #[node_cnt, 2*embed_dim]*[2*embed_dim, embed_dim] = [node_cnt, embed_dim]
                cur_message_layer_edge = tf.nn.relu(tf.matmul(merged_linear, p_node_conv3_edge))

                #[batch_size, embed_dim] * [embed_dim, embed_dim] = [batch_size, embed_dim], dense
                y_cur_message_layer_linear_edge = tf.matmul(tf.cast(y_cur_message_layer_edge, tf.float32), p_node_conv2_edge)
                #[[batch_size, embed_dim] [batch_size, embed_dim]] = [batch_size, 2*embed_dim], return tensed matrix
                y_merged_linear = tf.concat([y_edge_linear, y_cur_message_layer_linear_edge], 1)
                #[batch_size, 2*embed_dim]*[2*embed_dim, embed_dim] = [batch_size, embed_dim]
                y_cur_message_layer_edge = tf.nn.relu(tf.matmul(y_merged_linear, p_node_conv3_edge))
                        

            cur_message_layer_edge = tf.nn.l2_normalize(cur_message_layer_edge, axis=1)
            y_cur_message_layer_edge = tf.nn.l2_normalize(y_cur_message_layer_edge, axis=1)

        self.edge_embedding = cur_message_layer_edge
        ...
        y_potential_dual = y_cur_message_layer_edge
        #[batch_size, node_cnt] * [node_cnt, embed_dim] = [batch_size, embed_dim]
        # action_embed = tf.sparse_tensor_dense_matmul(tf.cast(self.action_select, tf.float32), cur_message_layer)
        #[batch_size, node_cnt] * [node_cnt, embed_dim] = [batch_size, embed_dim]
        action_embed = tf.sparse_tensor_dense_matmul(tf.cast(self.action_select, tf.float32), cur_message_layer)
        action_embed_edge = tf.sparse_tensor_dense_matmul(tf.cast(self.action_select_dual, tf.float32), cur_message_layer_edge)
        ...
        temp_edge = tf.matmul(tf.expand_dims(action_embed_edge, axis=2), tf.expand_dims(y_potential_dual, axis=1))
        # [batch_size, embed_dim]
        Shape = tf.shape(action_embed)
        Shape_edge = tf.shape(action_embed_edge)
        ...
        embed_s_a_edge = tf.reshape(tf.matmul(temp_edge, tf.reshape(tf.tile(cross_product, [Shape_edge[0], 1]), [Shape_edge[0], Shape_edge[1], 1])), Shape_edge)

        #[batch_size, 2 * embed_dim]
        last_output = embed_s_a
        last_output_edge = embed_s_a_edge

        if self.reg_hidden > 0:
            #[batch_size, 2*embed_dim] * [2*embed_dim, reg_hidden] = [batch_size, reg_hidden], dense
            hidden = tf.matmul(embed_s_a, h1_weight)
            hidden_edge = tf.matmul(embed_s_a_edge, h1_weight)
            #[batch_size, reg_hidden]
            last_output = tf.nn.tanh(hidden)
            last_output_edge = tf.nn.tanh(hidden_edge)
        ...
        last_output_edge = tf.concat([last_output_edge, self.aux_input_dual], 1)
        ...
        q_pred_edge = tf.matmul(last_output_edge, last_w)
        q_pred = tf.multiply(q_pred, self.node_actions)
        q_pred_edge = tf.multiply(self.target, self.edge_actions)

        # self.node_loss = q_pred
        # self.edge_loss = q_pred_edge

        q_pred = tf.add(q_pred, q_pred_edge)
        ...
        loss_recons_dual = 2 * tf.trace(tf.matmul(tf.transpose(cur_message_layer_edge), tf.sparse_tensor_dense_matmul(tf.cast(self.laplacian_param_dual,tf.float32), cur_message_layer_edge)))
        edge_num_dual = tf.sparse_reduce_sum(self.n2nsum_param_dual)
        loss_recons_dual = tf.divide(loss_recons_dual, edge_num_dual)
        ...
        if self.IsPrioritizedSampling:
            ...
            if self.IsHuberloss:
                ...
                loss_rl = tf.losses.mean_squared_error(self.target, q_pred) # + tf.losses.mean_squared_error(self.target_edge, q_pred_edge)

        loss = loss_rl + Alpha * (loss_recons + loss_recons_dual)
        #
        # loss = loss_rl
        ...
        '''generate Q value for edge'''
        #[node_cnt, batch_size] * [batch_size, embed_dim] = [node_cnt, embed_dim]
        rep_y_edge = tf.sparse_tensor_dense_matmul(tf.cast(self.rep_global_dual, tf.float32), y_potential_dual)
        #[[node_cnt, embed_dim], [node_cnt, embed_dim]] = [node_cnt, 2*embed_dim]
        # embed_s_a_all = tf.concat([cur_message_layer,rep_y],1)
        # # [node_cnt + edge_cnt, embed_dim, embed_dim]
        temp1_edge = tf.matmul(tf.expand_dims(cur_message_layer_edge, axis=2),tf.expand_dims(rep_y_edge, axis=1))
        # [node_cnt + edge_cnt, embed_dim]
        Shape1_edge = tf.shape(cur_message_layer_edge)
        # [batch_size, embed_dim], first transform
        embed_s_a_all_edge = tf.reshape(tf.matmul(temp1_edge, tf.reshape(tf.tile(cross_product,[Shape1_edge[0],1]),[Shape1_edge[0],Shape1_edge[1],1])),Shape1_edge)

        #[node_cnt, 2 * embed_dim]
        last_output_edge = embed_s_a_all_edge
        if self.reg_hidden > 0:
            #[node_cnt, 2 * embed_dim] * [2 * embed_dim, reg_hidden] = [node_cnt, reg_hidden1]
            hidden_edge = tf.matmul(embed_s_a_all_edge, h1_weight)
            #Relu, [node_cnt, reg_hidden1]
            last_output_edge = tf.nn.relu(hidden_edge)
            #[node_cnt, reg_hidden1] * [reg_hidden1, reg_hidden2] = [node_cnt, reg_hidden2]
            # last_output_hidden = tf.matmul(last_output1, h2_weight)
            # last_output = tf.nn.relu(last_output_hidden)

        #[node_cnt, batch_size] * [batch_size, aux_dim] = [node_cnt, aux_dim]
        rep_aux_edge = tf.sparse_tensor_dense_matmul(tf.cast(self.rep_global_dual, tf.float32), self.aux_input)

        #if reg_hidden == 0: , [[node_cnt, 2 * embed_dim], [node_cnt, aux_dim]] = [node_cnt, 2*embed_dim + aux_dim]
        #if reg_hidden > 0: , [[node_cnt, reg_hidden], [node_cnt, aux_dim]] = [node_cnt, reg_hidden + aux_dim]
        last_output_edge = tf.concat([last_output_edge,rep_aux_edge],1)

        #if reg_hidden == 0: , [node_cnt, 2 * embed_dim + aux_dim] * [2 * embed_dim + aux_dim, 1] = [node_cnt，1]
        #f reg_hidden > 0: , [node_cnt, reg_hidden + aux_dim] * [reg_hidden + aux_dim, 1] = [node_cnt，1]
        q_on_all_edge = tf.matmul(last_output_edge, last_w)

        # q_on_all = tf.concat([q_on_all, q_on_all_edge], axis=0)

        return loss,trainStep,q_pred,q_on_all, q_on_all_edge,tf.trainable_variables()
    ...
    def InsertGraph(self,g,is_test):
        ...
        if is_test:
            t = self.ngraph_test
            self.ngraph_test += 1
            g1, g1_dual = self.GenNetwork(g)
            self.TestSet.InsertGraph(t, g1, g1_dual)
        else:
            t = self.ngraph_train
            self.ngraph_train += 1
            g1, g1_dual = self.GenNetwork(g)
            self.TrainSet.InsertGraph(t, g1, g1_dual)
    ...
    def Run_simulator(self, int num_seq, double eps, TrainSet, int n_step):
        ...
        while n < num_seq:
            for i in range(num_env):
                # if the graph is null or game has terminal
                if self.env_list[i].graph.num_nodes == 0 or self.env_list[i].isTerminal():
                    # if the game is terminal, save playing process
                    if self.env_list[i].graph.num_nodes > 0 and self.env_list[i].isTerminal():
                        n = n + 1
                        self.nStepReplayMem.Add(self.env_list[i], n_step)
                    # random choose a graph and replace the finished graph
                    gid = np.random.randint(0, 10 ** 9 + 7) % self.ngraph_train
                    g_sample = TrainSet.Get(gid)
                    g_dual_sample = TrainSet.Get_dual(gid)
                    self.env_list[i].s0(g_sample, g_dual_sample, N_STEP_ACTION)
                    self.g_list[i] = self.env_list[i].graph
                    self.g_dual_list[i] = self.env_list[i].graph_dual
                    # build adjacency matrix
                    adj = np.zeros((self.env_list[i].graph.num_nodes, self.env_list[i].graph.num_nodes))
                    for j in range(self.env_list[i].graph.num_edges):
                        x, y = self.env_list[i].graph.edge_list[j]
                        adj[x, y] = adj[y, x] = self.env_list[i].graph_dual.nodes_weight[j]
                    self.adj_list[i] = adj
            if n >= num_seq:
                break
            # decide each graph choose random/best action
            Random = False
            if random.uniform(0,1) >= eps:
                # pred = self.PredictWithCurrentQNet(self.g_list, [env.action_list for env in self.env_list])
                pred = self.PredictWithCurrentQNet(self.g_list, self.g_dual_list, [env.covered_set for env in self.env_list], [env.edge_covered_set for env in self.env_list])
            else:
                Random = True
            ...
            for i in range(num_env):
                ...
                if a_t < self.env_list[i].graph.num_nodes:
                    self.adj_list[i][a_t, :] = 0
                    self.adj_list[i][:, a_t] = 0
                else:
                    edge_idx = a_t - self.env_list[i].graph.num_nodes
                    x, y = self.env_list[i].graph.edge_list[edge_idx]
                    self.adj_list[i][x, y] = self.adj_list[i][y, x] = 0
                # compute pairwise connectivity reward
                # compute robutness(counter == 0) -> update counter(counter == 1) -> compute pairwise connectivity(counter == 1)
                if self.env_list[i].save_state_cnt == 1:
                    exp_adj = scipy.linalg.expm(self.adj_list[i])
                    r_t = (np.sum(exp_adj) / self.env_list[i].graph.total_communicability) * 0.5
                    
                    self.env_list[i].updateReward(-r_t, a_t)
    #pass
    def PlayGame(self,int n_traj, double eps):
        self.Run_simulator(n_traj, eps, self.TrainSet, N_STEP_REPLAY)


    def SetupTrain(self, idxes, g_list, g_dual_list, covered_node, covered_edge, actions, target_node):
        self.m_y = target_node
        # self.m_y_edge = target_edge
        self.inputs['target'] = self.m_y
        # self.inputs['target_edge'] = self.m_y_edge
        prepareBatchGraph = PrepareBatchGraph.py_PrepareBatchGraph(aggregatorID)
        prepareBatchGraph.SetupTrain(idxes, g_list, covered_node, True, actions)
        ...
        prepareBatchGraphDual = PrepareBatchGraph.py_PrepareBatchGraph(aggregatorID)
        prepareBatchGraphDual.SetupTrain(idxes, g_dual_list, covered_edge, False, actions)
        self.inputs['action_select_dual'] = prepareBatchGraphDual.act_select
        self.inputs['rep_global_dual'] = prepareBatchGraphDual.rep_global
        self.inputs['n2nsum_param_dual'] = prepareBatchGraphDual.n2nsum_param
        self.inputs['laplacian_param_dual'] = prepareBatchGraphDual.laplacian_param
        self.inputs['subgsum_param_dual'] = prepareBatchGraphDual.subgsum_param
        self.inputs['node_input_dual'] = prepareBatchGraphDual.node_feat
        self.inputs['aux_input_dual'] = prepareBatchGraphDual.aux_feat

    def SetupPredAll(self, idxes, g_list, g_dual_list, covered_node, covered_edge):
        prepareBatchGraph = PrepareBatchGraph.py_PrepareBatchGraph(aggregatorID)
        prepareBatchGraph.SetupPredAll(idxes, g_list, covered_node, True)
        self.inputs['rep_global'] = prepareBatchGraph.rep_global
        self.inputs['n2nsum_param'] = prepareBatchGraph.n2nsum_param
        # self.inputs['laplacian_param'] = prepareBatchGraph.laplacian_param
        self.inputs['subgsum_param'] = prepareBatchGraph.subgsum_param
        ...
        prepareBatchGraphDual = PrepareBatchGraph.py_PrepareBatchGraph(aggregatorID)
        prepareBatchGraphDual.SetupPredAll(idxes, g_dual_list, covered_edge, False)
        self.inputs['rep_global_dual'] = prepareBatchGraphDual.rep_global
        self.inputs['n2nsum_param_dual'] = prepareBatchGraphDual.n2nsum_param
        self.inputs['subgsum_param_dual'] = prepareBatchGraphDual.subgsum_param
        self.inputs['node_input_dual'] = prepareBatchGraphDual.node_feat
        ...
        return prepareBatchGraph.idx_map_list, prepareBatchGraphDual.idx_map_list

    def Predict(self,g_list, g_dual_list,covered_node, covered_edge,isSnapSnot):
        ...
        for i in range(0, n_graphs, BATCH_SIZE):
            ...
            idx_map_list, idx_map_list_edge = self.SetupPredAll(batch_idxes, g_list, g_dual_list,covered_node, covered_edge)
            if isSnapSnot:
                result = self.session.run([self.q_on_allT, self.q_on_allT_edge], feed_dict={
                    ...
                    self.rep_global_dual: self.inputs['rep_global_dual'],
                    self.n2nsum_param_dual: self.inputs['n2nsum_param_dual'],
                    self.subgsum_param_dual: self.inputs['subgsum_param_dual'],
                    self.node_input_dual: self.inputs['node_input_dual'],
                    self.aux_input_dual: np.array(self.inputs['aux_input_dual']),
                })
            else:
                result = self.session.run([self.q_on_all, self.q_on_all_edge], feed_dict={
                    self.rep_global: self.inputs['rep_global'],
                    self.n2nsum_param: self.inputs['n2nsum_param'],
                    self.subgsum_param: self.inputs['subgsum_param'],
                    self.node_input: self.inputs['node_input'],
            ...
            raw_output, raw_output_edge = result[0], result[1]
            # print('Q value for node')
            # print(raw_output)
            # print('Q value for edge')
            # print(raw_output_edge)
            pos_node = 0
            pos_edge = 0
            pred = []
            # iterate through each item in the batch
            for j in range(i, i + bsize):
                idx_map, idx_map_edge = idx_map_list[j-i], idx_map_list_edge[j - i]
                cur_pred = np.zeros(len(idx_map) + len(idx_map_edge))
                # fill node's Q-value to the final array
                # negative in idx_map mean removed
                for k in range(len(idx_map)):
                    if idx_map[k] < 0:
                        cur_pred[k] = -inf
                    else:
                        cur_pred[k] = raw_output[pos_node]
                        pos_node += 1
                n = len(idx_map)
                # fill edge's Q-value to the final array
                for k in range(n, n + len(idx_map_edge)):
                    if idx_map_edge[k - n] < 0:
                        cur_pred[k] = -inf
                    else:
                        cur_pred[k] = raw_output_edge[pos_edge]
                        pos_edge += 1
                # check again
                for k in covered_node[j]:
                    cur_pred[k] = -inf
                for k in covered_edge[j]:
                    cur_pred[k + g_list[j].num_nodes] = -inf
                pred.append(cur_pred)
            assert (pos_node == len(raw_output) and pos_edge == len(raw_output_edge))
        return pred

    def PredictWithCurrentQNet(self,g_list, g_dual_list,covered_node, covered_edge):
        result = self.Predict(g_list, g_dual_list,covered_node, covered_edge,False)
        return result

    def PredictWithSnapshot(self,g_list, g_dual_list,covered_node, covered_edge):
        result = self.Predict(g_list, g_dual_list,covered_node, covered_edge,True)
        return result
    #pass
    ...
    def Fit(self):
        ...
        if ness:
            if self.IsDoubleDQN:
                double_list_pred = self.PredictWithCurrentQNet(sample.g_list, sample.g_dual_list, sample.list_s_primes, sample.list_s_primes_edge)
                double_list_predT = self.PredictWithSnapshot(sample.g_list, sample.g_dual_list, sample.list_s_primes, sample.list_s_primes_edge)
                list_pred = [a[self.argMax(b)] for a, b in zip(double_list_predT, double_list_pred)]
            else:
                list_pred = self.PredictWithSnapshot(sample.g_list, sample.g_dual_list, sample.list_s_primes, sample.list_s_primes_edge)
        
        list_target = np.zeros([BATCH_SIZE, 1])
        # print(sample.list_rt)
        for i in range(BATCH_SIZE):
            q_rhs = 0
            if (not sample.list_term[i]):
                if self.IsDoubleDQN:
        ...
        if self.IsPrioritizedSampling:
            ...
            return self.fit(sample.g_list, sample.g_dual_list, sample.list_st_node, sample.list_st_edge, sample.list_at,list_target)
    ...
    def fit_with_prioritized(self,tree_idx,ISWeights,g_list,covered,actions,list_target):
        ...
        for i in range(0,n_graphs,BATCH_SIZE):
            ...
            result = self.session.run([self.trainStep,self.TD_errors,self.loss],feed_dict={
                                        ...
                                        self.target_edge : self.inputs['target_edge']})
    ...
    def fit(self,g_list, g_dual_list,covered_node, covered_edge,actions,list_target):
        cdef double loss = 0.0
        cdef int n_graphs = len(g_list)
        cdef int i, j, bsize
        ...
        for i in range(0,n_graphs,BATCH_SIZE):
            ...
            node_actions = np.zeros((bsize, 1), dtype=float)
            edge_actions = np.zeros((bsize, 1), dtype=float)
            for j in range(bsize):
                if actions[batch_idxes[j]] < g_list[batch_idxes[j]].num_nodes:
                    node_actions[j] = 1.0
                    edge_actions[j] = 0.0
                else:
                    node_actions[j] = 0.0
            ...
            # self.node_actions = tf.convert_to_tensor(node_actions, dtype=tf.float32)
            # self.edge_actions = tf.convert_to_tensor(edge_actions, dtype=tf.float32)

            self.SetupTrain(batch_idxes, g_list, g_dual_list, covered_node, covered_edge, actions,list_target)
            result = self.session.run([self.loss,self.trainStep],feed_dict={
                                        ...
                                        self.node_input: self.inputs['node_input'],
                                        self.aux_input: np.array(self.inputs['aux_input']),
                                        self.action_select_dual : self.inputs['action_select_dual'],
                                        self.rep_global_dual : self.inputs['rep_global_dual'],
                                        self.n2nsum_param_dual : self.inputs['n2nsum_param_dual'],
                                        self.laplacian_param_dual : self.inputs['laplacian_param_dual'],
                                        self.subgsum_param_dual : self.inputs['subgsum_param_dual'],
                                        self.node_input_dual: self.inputs['node_input_dual'],
                                        self.aux_input_dual: np.array(self.inputs['aux_input_dual']),
                                        self.target : self.inputs['target'],
    ...
    def Train(self):
        ...
        for iter in range(MAX_ITERATION):
            ...
            if iter % 300 == 0:
                ...
                f_out.write('%f\n'%(N_end - N_start))   #write vc into the file
                f_out.flush()
    ...
    def Evaluate_action(self, g, g_dual):
        cdef int t
        self.ClearTestGraphs()
        t = self.ngraph_test
        self.TestSet.InsertGraph(t, g, g_dual)
        sol = self.GetSolution(0)
        return sol
    ...
    def GetSolution(self, int gid, int step=1):
        g_list = []
        g_dual_list = []
        self.test_env.s0(self.TestSet.Get(gid), self.TestSet.Get_dual(gid), 1)
        g_list.append(self.test_env.graph)
        g_dual_list.append(self.test_env.graph_dual)
        sol = []
        start = time.time()
        cdef int iter = 0
        cdef int new_action
        while (not self.test_env.isTerminal()):
            ...
            iter += 1
            list_pred = self.PredictWithCurrentQNet(g_list, g_dual_list, [self.test_env.covered_set], [self.test_env.edge_covered_set])
    ...
    def GenNetwork(self, g):    #networkx2four
        ...
        edge_weights = [np.random.random() for _ in range(len(edges))]
        # initial node weights
        weights = [0] * len(nodes)
        # assign node weights, equals 0.5 * sum(incident edge's weight)
        for i, (a, b) in enumerate(list(edges)):
            weights[a] += edge_weights[i] / 2
            weights[b] += edge_weights[i] / 2
        ...
        if len(edges) > 0:
            ...
            ED = np.array([0] * len(edges))
        else:
            A = np.array([0])
            B = np.array([0])
            W = np.array([0])
            EW = np.array([0])
        # compute initail connectivity
        g_numpy = nx.to_numpy_array(g)
        total_pc = np.sum(linalg.expm(g_numpy)) - len(nodes)
        return graph.py_Graph(len(nodes), len(edges), A, B, W, total_pc, D), graph.py_Graph(len(edges), len(EA), EA, EB, EW, 0.0, ED)
    ...
    def HXA(self, g, method):
        ...
        g1, g1_dual = self.GenNetwork(g)
        Robustness = self.utils.getRobustness(g1, solutions)
        return Robustness
