...
cdef class py_MvcEnv:
    ...
    def s0(self,_g, _g_dual, nstep):
        ...
        deref(self.inner_Graph).degrees = _g.degrees
        deref(self.inner_Graph).total_nodes_weight =_g.total_nodes_weight
        deref(self.inner_Graph).total_communicability = _g.total_communicability
        deref(self.inner_Graph).edge_map = _g.edge_map

        self.inner_Graph_dual = shared_ptr[Graph](new Graph())
        deref(self.inner_Graph_dual).num_nodes = _g_dual.num_nodes
        deref(self.inner_Graph_dual).num_edges = _g_dual.num_edges
        deref(self.inner_Graph_dual).edge_list = _g_dual.edge_list
        deref(self.inner_Graph_dual).adj_list = _g_dual.adj_list
        deref(self.inner_Graph_dual).nodes_weight = _g_dual.nodes_weight
        deref(self.inner_Graph_dual).degrees = _g_dual.degrees
        deref(self.inner_Graph_dual).total_nodes_weight = _g_dual.total_nodes_weight
        deref(self.inner_Graph_dual).total_communicability = _g_dual.total_communicability
        deref(self.inner_Graph_dual).edge_map = _g_dual.edge_map

        deref(self.inner_MvcEnv).s0(self.inner_Graph, self.inner_Graph_dual, nstep)
    ...
    def updateReward(self, r, a):
        return deref(self.inner_MvcEnv).updateReward(r, a)
    ...
    @property
    def graph_dual(self):
        return self.G2P(deref(self.inner_Graph_dual))
    ...
    @property
    def edge_covered_set(self):
        return deref(self.inner_MvcEnv).edge_covered_set
    ...
    @property
    def avail_list_edge(self):
        return deref(self.inner_MvcEnv).avail_list_edge

    @property
    def save_state_cnt(self):
        return deref(self.inner_MvcEnv).save_state_cnt

    @property
    def save_every_nstep(self):
        return deref(self.inner_MvcEnv).save_every_nstep

    @property
    def current_weight(self):
        return deref(self.inner_MvcEnv).current_weight
