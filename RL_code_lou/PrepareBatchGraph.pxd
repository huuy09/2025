...
cdef extern from "./src/lib/PrepareBatchGraph.h":
    ...
    cdef cppclass PrepareBatchGraph:
        ...
        void SetupTrain(vector[int] idxes,vector[shared_ptr[Graph] ] g_list,vector[vector[int]] covered, int is_node,const int* actions)except+
        void SetupPredAll(vector[int] idxes,vector[shared_ptr[Graph] ] g_list,vector[vector[int]] covered, int is_node)except+
