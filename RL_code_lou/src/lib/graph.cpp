...
Graph::Graph() : num_nodes(0), num_edges(0)
{
    ...
    total_communicability = 0.0;
    edge_map.clear();
}

Graph::Graph(const int _num_nodes, const int _num_edges, const int* edges_from, const int* edges_to,const double* _nodes_weight, const double _total_communicability, const int* _degrees) //, const double* _edges_weight)
        : num_nodes(_num_nodes), num_edges(_num_edges)
{
    ...
    degrees.clear();
    total_nodes_weight = 0.0;
    total_communicability = _total_communicability;
    for (int i = 0; i < num_nodes; ++i)
    {
        adj_list[i].clear();
        nodes_weight.push_back(_nodes_weight[i]);
        degrees.push_back(_degrees[i]);
    ...
    for (int i = 0; i < num_edges; ++i)
    {
        ...
        edge_list[i] = edge;
        edge_map[edge] = i;
        // edges_weight.push_back(_edges_weight[i]);
    }
//    for (int i = 0; i < num_nodes; ++i)
...
void GSet::InsertGraph(int gid, std::shared_ptr<Graph> graph, std::shared_ptr<Graph> graph_dual)
{
    assert(graph_pool.count(gid) == 0);

    graph_pool[gid] = graph;
    graph_dual_pool[gid] = graph_dual;
}
...
std::shared_ptr<Graph> GSet::Get_dual(int gid)
{
    assert(graph_pool.count(gid));
    return graph_dual_pool[gid];
}
...
//    printf("graph_pool_size:%d",graph_pool.size());
    ...
    // assert(graph_dual_pool[gid]);
    return graph_pool[gid];
}

// std::shared_ptr<Graph> GSet::Sample_dual()
// {
// //    printf("graph_pool_size:%d",graph_pool.size());
//     assert(graph_dual_pool.size());
// //    printf("graph_pool_size:%d",graph_pool.size());
//     int gid = rand() % graph_dual_pool.size();
//     assert(graph_dual_pool[gid]);
//     return graph_dual_pool[gid];
// }
