...
void Memory::Add(std::shared_ptr<MvcEnv> env,int n_step)
{
    ...
    for (int i = 0; i < num_steps; ++i)
    {
        ...
        transition->s_t_node = state_node;
        transition->s_t_edge = state_edge;
        transition->s_prime = s_prime;
        transition->s_prime_edge = s_prime_edge;
...
//        sum += result->ISWeights[i];
        ...
        result->list_st_node[i] =leafResult->data->s_t_node;
        result->list_st_edge[i] =leafResult->data->s_t_edge;
        result->list_s_primes[i] =leafResult->data->s_prime;
        result->list_s_primes_edge[i] =leafResult->data->s_prime_edge;
