    ...
    {             
        ...
        {   
            ...
            if (is_node){
                ...
                if (act >= g->num_nodes){
                    act_select->rowIndex.push_back(i);
                    act_select->colIndex.push_back(node_cnt + idx_map[act - g->num_nodes]);
                    act_select->value.push_back(1.0);
                }
            }
...
void PrepareBatchGraph::SetupTrain(std::vector<int> idxes,
                           ...
                           const int* actions)
{
    SetupGraphInput(idxes, g_list, covered, is_node, actions);
}
...
void PrepareBatchGraph::SetupPredAll(std::vector<int> idxes,
                           ...
                           bool is_node)
{
    SetupGraphInput(idxes, g_list, covered, is_node, nullptr);
}
