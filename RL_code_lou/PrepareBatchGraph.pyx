...
cdef class py_PrepareBatchGraph:
    ...
    #         gc.collect()
    def SetupTrain(self,idxes,g_list,covered, is_node,list actions):
        ...
        for i in range(len(actions)):
            refint[i] = actions[i]
        deref(self.inner_PrepareBatchGraph).SetupTrain(idxes,inner_glist,covered, is_node,refint)
        free(refint)

    def SetupPredAll(self,idxes,g_list,covered, is_node):
        cdef shared_ptr[Graph] inner_Graph
        cdef vector[shared_ptr[Graph]] inner_glist
        for _g in g_list:
            # inner_glist.push_back(_g.inner_Graph)
            ...
            deref(inner_Graph).total_nodes_weight=_g.total_nodes_weight
            inner_glist.push_back(inner_Graph)
        deref(self.inner_PrepareBatchGraph).SetupPredAll(idxes,inner_glist,covered, is_node)
